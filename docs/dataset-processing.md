...
Phase 3 supports CSV upload, private Storage, and deterministic column profiling.
...
10. Activity events are recorded for upload started, ready, or processing failed.

Supabase Storage and PostgreSQL writes are not atomic together. If file upload succeeds but later parsing or metadata writes fail, the app preserves the private file and marks the dataset as `failed` with a useful reason. No raw CSV rows are copied into PostgreSQL.
