...
`data_quality_issues` stores workspace, dataset, optional column, controlled issue type, severity, status, assignment, creator, source, compact detection metadata, automated key, timestamps, resolution actor, and optional resolution note.

`issue_comments` stores plain-text issue discussion content, limited to 5,000 characters.
...
- Authenticated clients have read-only grants on issues and comments. Server Actions validate membership and use a server-only service-role client for narrow writes. Activity events have no direct client mutation policy.
...
`public.record_dataset_activity(target_dataset_id, target_event_type, event_metadata)` originally recorded upload lifecycle events. Phase 5 revokes client execution; validated server-only upload code now writes these events. Payloads contain operational metadata such as filename, byte size, row count, column count, or a bounded failure reason. They must not contain secrets or raw CSV content.
...
Automated quality issues use `source = 'automated'`, a deterministic `automated_issue_key`, and compact `detection_metadata`. The uploader remains `created_by`. Reprocessing retains existing findings by key so workflow state and comments survive; only new keys are inserted. Manual issues are untouched. See `docs/issue-workflow.md` for roles and transitions.
